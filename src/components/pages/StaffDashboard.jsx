...
//   await axios.post(
//     `${import.meta.env.VITE_API_URL}/api/incidents/${id}/send-otp`,
//     {},
//     { headers: { Authorization: `Bearer ${token}` } }
//   );
...
//   await axios.post(
//     `${import.meta.env.VITE_API_URL}/api/incidents/${id}/resend-otp`,
//     {},
//     { headers: { Authorization: `Bearer ${token}` } }
//   );
...
const TicketSkeleton = () => (
  <div className="bg-slate-900/40 border border-white/10 rounded-2xl overflow-hidden p-5 flex flex-col gap-4">
...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on("ticket_created", handleRefresh);
    socket.on("ticket_assigned", handleRefresh);
    socket.on("ticket_department_updated", handleRefresh);
    socket.on("ticket_resolved", handleRefresh);

    return () => {
      socket.off("notification");
      socket.off("ticket_created", handleRefresh);
      socket.off("ticket_assigned", handleRefresh);
      socket.off("ticket_department_updated", handleRefresh);
      socket.off("ticket_resolved", handleRefresh);
    };
  }, [user]);
  ...
  const getAiSuggestion = async (ticket) => {
    try {
      ...
      setAiSuggestion((prev) => ({ ...prev, [ticket._id]: res.data.suggestion }));
    } catch (err) {
      console.error("AI ERROR:", err);
    }
  };
  ...
  const sendOTP = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.post(`${import.meta.env.VITE_API_URL}/api/incidents/${id}/send-otp`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const downloadReport = async (ticketId) => {
    const token = localStorage.getItem("token");
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/incidents/${ticketId}/report`, {
      headers: { Authorization: `Bearer ${token}` },
      responseType: "blob",
    });
    const url = window.URL.createObjectURL(new Blob([res.data]));
  ...
  const getStatusBadge = (status) => {
    ...
    const icons = {
      ...
      'Closed': <AlertCircle className="w-3.5 h-3.5" />,
      'Reopened': <AlertCircle className="w-3.5 h-3.5" />,
      'Open': <Clock className="w-3.5 h-3.5" />
    };
    return (
      <span className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border", styles[status] || styles['Open'])}>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <header className="sticky top-0 z-40 backdrop-blur-xl bg-slate-950/80 border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold bg-gradient-to-r from-white to-slate-400 bg-clip-text text-transparent hidden sm:block">
              Staff Portal
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            <div className="relative group">
              <button className="flex items-center gap-3 bg-white/5 hover:bg-white/10 px-2 sm:px-3 py-1.5 rounded-xl border border-white/10 transition-all">
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white text-xs font-bold shadow-inner">
                  {user?.full_name?.charAt(0)}
                </div>
                <div className="hidden md:block text-left">
                  <p className="text-sm font-medium text-white leading-none">{user?.full_name}</p>
                  <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-wider">{user?.department}</p>
                </div>
                <ChevronDown className="w-4 h-4 text-slate-500" />
              </button>
              <div className="absolute right-0 mt-2 w-56 bg-slate-900 border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right z-50 p-2">
                <button onClick={() => navigate("/staff/profile")} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-slate-300 hover:bg-white/5 rounded-lg transition-colors">
                  <User className="w-4 h-4" /> Profile
                </button>
                <hr className="my-1 border-white/5" />
                <button onClick={() => { logout(); navigate("/login"); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
                  <LogOut className="w-4 h-4" /> Logout
                </button>
              </div>
            </div>
      ...
      <div className="fixed top-20 right-4 z-50 flex flex-col gap-3 pointer-events-none max-w-sm w-full px-4 sm:px-0">
        <AnimatePresence>
          {notifications.map((n, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}
              className="pointer-events-auto bg-slate-900/90 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-2xl flex gap-3">
              <div className="bg-indigo-500/20 p-2 rounded-lg h-fit text-indigo-400"><Bell className="w-4 h-4" /></div>
              <div>
                <h4 className="font-bold text-white text-sm">{n.title}</h4>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Assigned Workload</h2>
            <p className="text-slate-400 text-sm mt-1">Manage and resolve assigned support tickets</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <div className="relative flex-1 md:flex-none">
              ...
              <input 
                ...
                className="w-full md:w-64 bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40 transition-all"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <select 
              className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/40"
              value={filterStatus}
        ...
        {loading && tickets.length === 0 ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="text-center py-24 bg-white/5 rounded-3xl border border-white/5 border-dashed">
            <div className="w-20 h-20 bg-slate-900 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-white/5">
              <Ticket className="w-10 h-10 text-slate-700" />
            </div>
            <p className="text-slate-400 text-lg font-medium">No tickets found</p>
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible"
            ...
            {filteredTickets.map((ticket) => (
              <motion.div key={ticket._id} variants={itemVariants}
                className="group flex flex-col bg-slate-900/40 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-indigo-500/40 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all duration-300">
                ...
                <div className="p-5 flex-1 flex flex-col gap-5">
                  <p className="text-slate-400 text-sm line-clamp-2 leading-relaxed">{ticket.description}</p>

                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-indigo-400 flex items-center gap-1.5">
                        <Bot className="w-4 h-4" /> AI Smart Assist
                      </span>
                      <button onClick={() => getAiSuggestion(ticket)} className="text-[10px] text-indigo-300 hover:text-white underline underline-offset-2 decoration-indigo-500/50">
                        Get Suggestion
                      </button>
                    </div>
                    <AnimatePresence mode="wait">
                      {aiSuggestion[ticket._id] ? (
                        <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }}
                          className="text-xs text-slate-300 bg-indigo-500/5 p-3 rounded-xl border border-indigo-500/10 italic leading-relaxed">
                          "{aiSuggestion[ticket._id]}"
                        </motion.div>
                      ) : (
                        <div className="h-10 flex items-center justify-center border border-white/5 border-dashed rounded-xl">
                          <p className="text-[10px] text-slate-600">Click to generate resolution steps</p>
                        </div>
                      )}
                    </AnimatePresence>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="bg-black/20 rounded-xl p-3 h-24 overflow-y-auto space-y-2 custom-scrollbar">
                      {ticket.comments?.length > 0 ? (
                        ticket.comments.map((c, i) => (
                          <div key={i} className="text-[11px] leading-snug">
                            <span className="font-bold text-indigo-400">{c.role}:</span>{" "}
                            <span className="text-slate-400">{c.message}</span>
                          </div>
                        ))
                      ) : (
                        <p className="text-[10px] text-slate-600 italic text-center mt-4">No activity recorded</p>
                      )}
                    </div>
                    <div className="relative mt-1">
                      <input type="text" value={commentText[ticket._id] || ""} 
                        onChange={(e) => setCommentText(p => ({ ...p, [ticket._id]: e.target.value }))}
                        placeholder="Add internal note..." onKeyDown={(e) => e.key === 'Enter' && addComment(ticket._id)}
                        className="w-full bg-white/5 border border-white/10 rounded-xl pl-3 pr-10 py-2 text-xs text-white focus:outline-none focus:border-indigo-500/50 transition-all" />
                      <button onClick={() => addComment(ticket._id)} className="absolute right-1 top-1 p-1.5 text-indigo-400 hover:text-white transition-colors">
                ...
                <div className="p-4 bg-white/5 border-t border-white/5 flex flex-col gap-2">
                  <div className="grid grid-cols-2 gap-2">
                    {(ticket.status === "Open" || ticket.status === "Reopened") && (
                      <button onClick={() => updateStatus(ticket._id, "In Progress")}
                        className="flex items-center justify-center gap-2 px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/20">
                        <Loader2 className="w-3.5 h-3.5" /> Start
                      </button>
                    )}
                    {ticket.status === "In Progress" && (
                      <button onClick={() => updateStatus(ticket._id, "Resolved")}
                        className="flex items-center justify-center gap-2 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-emerald-600/20">
                        <CheckCircle2 className="w-3.5 h-3.5" /> Resolve
                      </button>
                    )}
                    <button onClick={() => downloadReport(ticket._id)}
                      className="flex items-center justify-center gap-2 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-xl border border-white/5 transition-all">
                  ...
                  {ticket.status === "Resolved" && (
                    <div className="flex items-center gap-2 pt-2">
                       <button onClick={() => sendOTP(ticket._id)}
                        className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-amber-600/10 hover:bg-amber-600/20 text-amber-500 text-[10px] font-bold rounded-xl border border-amber-600/20 transition-all">
                        <Key className="w-3 h-3" /> Send OTP
                      </button>
                      <button onClick={() => sendOTP(ticket._id)}
                        className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-xl border border-white/5 transition-all">
