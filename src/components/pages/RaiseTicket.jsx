...
const getRootCause = async () => {
  ...
  try {
    ...
    setRootCause(res.data.rootCause || "No root cause found");
  } catch (err) {
    console.error("Root cause error:", err?.response?.data || err.message);
    setRootCause("⚠️ Failed to fetch root cause");
  }
};
...
>
            ...
            <button type="button" onClick={getRootCause} className="bg-purple-600 p-2 rounded">
Get Root Cause
</button>
